...
const handleImageChange = (event: Event) => {
  ...
  if (!['image/jpeg', 'image/png', 'image/webp'].includes(file.type)) {
    imageError.value = 'Please select a JPG, PNG or WebP image.'
    input.value = ''
    return
  }

  if (file.size > 2 * 1024 * 1024) {
    imageError.value = 'The image must not be larger than 2 MB.'
    input.value = ''
    return
  }
...
<template>
  ...
  <main v-else class="customer-account">
    <section class="account-banner">
      <div class="container">
        <div><span>My account</span>
          ...
          <p>Manage your orders, saved details and shopping activity.</p>
        </div>
    ...
    <div class="shop-breadcrumb">
      <div class="container">
        <NuxtLink to="/">Home</NuxtLink><i class="bi bi-chevron-right"></i><span>My Account</span>
      </div>
    </div>
      ...
      <div class="account-mobile-tabs">
        <button v-for="section in sections" :key="section.id" type="button"
          :class="{ active: activeSection === section.id }" @click="activeSection = section.id"><i class="bi"
            :class="section.icon"></i>{{ section.label }}</button>
      </div>

      <div class="account-layout">
        <aside class="account-sidebar">
          <div class="account-user">
            <div>{{ initials }}</div><span><strong>{{ displayName }}</strong><small>{{ user?.email }}</small></span>
          </div>
          <nav>
            <button v-for="section in sections" :key="section.id" type="button"
              :class="{ active: activeSection === section.id }" @click="activeSection = section.id"><i class="bi"
          ...
          <div class="account-support"><i class="bi bi-headset"></i>
            <div><small>Need assistance?</small><strong>We are here to help</strong>
        ...
        <div class="account-content">
          <template v-if="activeSection === 'dashboard'">
            <div class="account-heading">
              <div><small>Overview</small>
                <h2>Your account at a glance</h2>
              </div>
              <NuxtLink to="/shop">Continue shopping <i class="bi bi-arrow-right"></i></NuxtLink>
            </div>
            ...
            <div class="account-panel">
              <div class="account-panel-head">
                <div><small>Recent activity</small>
                  <h3>Recent orders</h3>
                </div><button type="button" @click="activeSection = 'orders'">View all orders</button>
              </div>
            ...
            <div class="account-bottom-grid">
              <div class="account-panel account-address-card">
                <div class="account-panel-head">
                  <div><small>Default address</small>
              ...
              <div class="account-help-card"><i class="bi bi-shield-check"></i>
                ...
                <p>Your account and payment information are protected.</p>
                <NuxtLink to="/contact">Get customer support <i class="bi bi-arrow-right"></i></NuxtLink>
              </div>
            </div>
          </template>
          ...
          <template v-else-if="activeSection === 'orders'">
            <div class="account-heading">
              <div><small>Order history</small>
                <h2>My orders</h2>
                <p>Track current purchases and review previous orders.</p>
              </div>
          ...
          <template v-else-if="activeSection === 'address'">
            <div class="account-heading">
              <div><small>Delivery details</small>
                <h2>Saved addresses</h2>
                <p>Manage where your NOVACART orders are delivered.</p>
              </div><button class="account-add-button" type="button"><i class="bi bi-plus-lg"></i> Add address</button>
            </div>
            <div class="address-grid">
              <article><span>Default</span><i class="bi bi-house"></i>
                <h3>Home</h3>
                <address><strong>Sumon Rahman</strong><small>House 12, Road 4<br>Dhanmondi, Dhaka
                    1209<br>Bangladesh<br>+880 1712-345678</small></address>
                <div><button type="button">Edit</button><button type="button">Remove</button></div>
              </article><button class="address-add-card" type="button"><i class="bi bi-plus-lg"></i><strong>Add a new
                  address</strong><small>Save another delivery location</small></button>
            </div>
          </template>

          <template v-else>
            <div class="account-heading">
              <div><small>Personal information</small>
...
.account-sidebar nav button>i:first-child,
...
.account-sidebar nav button>i:last-child {
  font-size: .65rem
}
...
.account-sidebar nav button.active>i:first-child {
  color: var(--brand)
}

.account-sidebar nav a b {
  display: grid;
  width: 21px;
  height: 21px;
  border-radius: 50%;
...
.account-sidebar nav .account-logout {
  margin-top: 7px;
...
.account-summary-grid>a,
.account-summary-grid>button {
  display: flex;
  align-items: center;
  gap: 13px;
  min-width: 0;
...
.account-summary-grid>a>i:first-child,
.account-summary-grid>button>i:first-child {
  display: grid;
  flex: 0 0 43px;
  width: 43px;
  height: 43px;
...
.account-summary-grid>a>i:last-child,
.account-summary-grid>button>i:last-child {
  color: #a1aaa5
}
...
.account-order-row span small {
  display: block;
  color: #9ba49f;
  font-size: .63rem
}

.account-order-row button {
  border: 0;
  background: transparent
}
...
.account-address-card address strong {
  margin-bottom: 3px;
  color: var(--ink)
}
...
.address-grid article>i {
  color: var(--brand);
  font-size: 1.5rem
}

.address-grid article h3 {
  margin: 14px 0
}
...
.address-grid address strong,
.address-grid address small {
  display: block
}
...
.address-grid article>div {
  display: flex;
  gap: 15px;
  margin-top: 20px
}

.address-grid article button {
  border: 0;
  background: transparent;
  padding: 0;
  color: var(--brand);
...
.address-add-card strong {
  margin: 12px 0 4px;
  color: var(--ink)
}

.address-add-card small {
  font-size: .7rem
}
...
.profile-photo-editor {
  ...
  background: linear-gradient(110deg, #fff8f6 0%, #fbfcfb 58%, #f3f7f5 100%);
  padding: 24px 28px
}
...
.profile-photo-preview {
  ...
  place-items: center;
  background: #ffe8e2;
  box-shadow: 0 8px 26px rgba(23, 33, 29, .12), 0 0 0 1px #dde3df;
  color: var(--brand);
  font-size: 1.75rem;
  font-weight: 800
}
...
.profile-photo-camera {
  ...
  box-shadow: 0 4px 12px rgba(255, 89, 65, .3);
  color: #fff;
  cursor: pointer
}
...
.profile-photo-button:hover {
  box-shadow: 0 8px 18px rgba(255, 89, 65, .22);
  transform: translateY(-1px)
}

.profile-photo-button:focus-visible,
.profile-photo-camera:focus-visible {
  outline: 3px solid rgba(255, 89, 65, .25);
  outline-offset: 3px
}
...
.profile-photo-name { color: #267647 }
.profile-photo-error { color: #c43d31 }
...
.account-profile-form input:focus {
  ...
  box-shadow: 0 0 0 3px rgba(255, 89, 65, .08)
}
