...
const money = (value: number | string) => `৳${new Intl.NumberFormat('en-BD', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(Number(value))}`
...
const statusRank: Record<string, number> = { pending: 0, confirmed: 0, processing: 1, shipped: 2, delivered: 3 }
const currentRank = computed(() => statusRank[order.value?.order_status ?? 'pending'] ?? 0)
const steps = [
  { status: 'confirmed', label: 'Confirmed', icon: 'bi-check-lg', fallback: 'Order received' },
...
const stepNote = (step: typeof steps[number]) => {
  const history = order.value?.status_histories.find(entry => entry.status === step.status || (step.status === 'confirmed' && entry.status === 'pending'))
