      ...
      <aside class="account-visual">
        <div class="account-visual-content">
          ...
          <div class="account-benefit-list"><span><i class="bi bi-box-seam"></i><b>Track every order</b><small>Updates
                ...
                favourites</b><small>Keep products ready for later</small></span><span><i
                class="bi bi-lightning-charge"></i><b>Checkout faster</b><small>Continue where you left
                off</small></span></div>
        </div>
        <div class="account-visual-footer"><span><i class="bi bi-shield-check"></i> Secure &amp; private</span></div>
      </aside>
      <section class="account-form-panel">
        <div class="account-form-wrap">
          ...
          <form class="account-form" @submit.prevent="submit">
            <label class="account-field" :class="{ invalid: errors.email }"><span>Email address</span>
              <div><i class="bi bi-envelope"></i><input v-model="form.email" type="email" autocomplete="email"
                  placeholder="you@example.com" required></div>
            </label>
            <label class="account-field"><span>Password</span>
              <div><i class="bi bi-lock"></i><input v-model="form.password" :type="showPassword ? 'text' : 'password'"
                  autocomplete="current-password" placeholder="Enter your password" required><button type="button"
                  data-password-toggle @click="showPassword = !showPassword"><i
                    :class="showPassword ? 'bi bi-eye-slash' : 'bi bi-eye'"></i></button></div>
            </label>
            <div class="account-form-row"><label><input v-model="form.remember" type="checkbox"> Remember me</label>
              <NuxtLink to="/forgot-password">Forgot password?</NuxtLink>
            </div>
            <button class="account-submit" :disabled="loading"><span>{{ loading ? 'Signing in...' : 'Sign in'
                }}</span><i class="bi bi-arrow-right"></i></button>
          </form>
          <p class="account-switch">Don’t have an account? <NuxtLink :to="{ path: '/register', query: { redirect } }">
              Create one</NuxtLink>
          </p>
        </div>
      </section>
